...
@section('content')
	<section class="content-header">
     ...
     <button type="button" class="btn btn-primary pull-right Add_foods"><i class="fa fa-plus"></i> Add Foods</button>
      </h1>
    </section>
    ...
    <section class="content">
      ...
      <div class="row">
       ...
       <div class="modal fade" id="AddfoodsModal">
    ...
    <div class="modal fade" id="Edit_food_class_modal">
    <div class="modal-dialog modal-lg">
    <div class="box box-primary" id="edit_foods_modal">
...
$("#datepicker").datepicker({
  ...
  currentDate = $( "#datepicker" ).datepicker( "getDate" );
   get_foods_dates(convert(currentDate));
});
});
...
function get_foods_dates(date) {
  $.ajax({
        type: "GET",
        url: '/management/foods_dates/'+date,
        beforeSend: function(){ ColoredLoader('day_food_div',"Loading..."); },
        complete: function(){ $('#day_food_div').preloader('remove');},
        success: function (data) {
          $("#day_food_div").replaceWith(data);
        }
         });
}
...
$(".Add_foods").click(function() {
     $('#AddfoodsModal').modal('show');
  });
       ...
       e.preventDefault();
        ...
        var formdata = new FormData(this);
            $.ajax({
                ...
                contentType: false,
                data: formdata,
               beforeSend: function(){ ColoredLoader('create_food_form',"Saving..."); },
              complete: function(){ $('#create_food_form').preloader('remove');},
              success: function (data) {
                  $("#foods_table").append(data);
                  $('.select2').val('').trigger("change");
                    snackbartext('success','food Saved Succcesfully');
                   $('.close').click();
                     document.getElementById("create_food_form").reset();
                   get_foods_dates(convert(pick_date));
                 },
...
$(document).on('submit', '#edit_food_form', function(e){
      ...
      var url =  $('#edit_food_form').attr('action');
        ...
        var formdata = new FormData(this);
            $.ajax({
                ...
                contentType: false,
                data: formdata,
               beforeSend: function(){ ColoredLoader('edit_food_form',"Saving..."); },
              complete: function(){ $('#edit_food_form').preloader('remove');},
              success: function (data) {
                  $("#foods_table").append(data);
                    snackbartext('success','food Saved Succcesfully');
                   $('.close').click();
                  $('.select2').val('').trigger("change");
                   get_foods_dates(convert(pick_date_edit));
  ...
  $(document).on('click', '.editTr', function(){
    ...
    $.ajax({
        type: "GET",
        url: '/management/food/'+ id +'/edit',
        success: function (data) {
            $("#edit_foods_modal").replaceWith(data);
...
$("#edit-textarea").wysihtml5();
  ...
  $(document).on('click', '.deleteFood', function () {
    ...
    var $tr = $(this).closest("li#food_list_"+id);
    swal({
